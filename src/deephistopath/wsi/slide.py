...
SRC_TRAIN_DIR = os.path.join(BASE_DIR, "doe-globus-pdx-data")  # ap TODO this shouldn't be hard coded
SRC_TRAIN_EXT = "svs"
...
def get_tile_image_path(tile):
  ...
  tile_path = os.path.join(TILE_DIR,
                           str(t.slide_num) + "-" + TILE_SUFFIX + "-r%d-c%d-x%d-y%d-w%d-h%d" % (
                           t.r, t.c, t.o_c_s, t.o_r_s, t.o_c_e - t.o_c_s, t.o_r_e - t.o_r_s) + "." + DEST_TRAIN_EXT)
  return tile_path
...
def get_training_image_path(slide_number, large_w=None, large_h=None, small_w=None, small_h=None):
  ...
  ## ap
  if large_w is None and large_h is None and small_w is None and small_h is None:
    wildcard_path = os.path.join(DEST_TRAIN_DIR, str(slide_number) + "*." + DEST_TRAIN_EXT)
    img_path = glob.glob(wildcard_path)[0]
  else:
...
def get_training_thumbnail_path(slide_number, large_w=None, large_h=None, small_w=None, small_h=None):
  ...
  ## ap
  if large_w is None and large_h is None and small_w is None and small_h is None:
    wilcard_path = os.path.join(DEST_TRAIN_THUMBNAIL_DIR, str(slide_number) + "*." + THUMBNAIL_EXT)
    img_path = glob.glob(wilcard_path)[0]
  else:
    img_path = os.path.join(DEST_TRAIN_THUMBNAIL_DIR, str(slide_number) + "-" + str(
      SCALE_FACTOR) + "x-" + DEST_TRAIN_SUFFIX + str(
      large_w) + "x" + str(large_h) + "-" + str(small_w) + "x" + str(small_h) + "." + THUMBNAIL_EXT)
...
def get_filter_image_filename(slide_number, filter_number, filter_name_info, thumbnail=False):
  ...
  ## img_filename = TRAIN_PREFIX + padded_sl_num + "-" + padded_fi_num + "-" + FILTER_SUFFIX + filter_name_info + "." + ext  ## ap
  img_filename = padded_sl_num + "-" + padded_fi_num + "-" + FILTER_SUFFIX + filter_name_info + "." + ext  ## ap
  return img_filename
...
def get_filter_image_result(slide_number):
  ...
  training_img_path = get_training_image_path(slide_number)
  large_w, large_h, small_w, small_h = parse_dimensions_from_image_filename(training_img_path)
  img_path = os.path.join(FILTER_DIR, str(slide_number) + "-" + str(
    SCALE_FACTOR) + "x-" + FILTER_SUFFIX + str(large_w) + "x" + str(large_h) + "-" + str(small_w) + "x" + str(
    small_h) + "-" + FILTER_RESULT_TEXT + "." + DEST_TRAIN_EXT)
...
def slide_to_scaled_pil_image(slide_number):
  ...
  ## print('Best level to downsample:', level)
  whole_slide_image = slide.read_region((0, 0), level, slide.level_dimensions[level])
  whole_slide_image = whole_slide_image.convert("RGB")  # PIL.Image method
...
def slide_stats(datapath):  ## ap
  ...
  train_images = glob.glob(os.path.join(datapath, '*.svs'))  ## ap

  num_train_images = get_num_training_slides()
  slide_stats = []
  ## for slide_num in range(1, num_train_images + 1):  ## ap
  for slide_filepath in train_images:  ## ap
    slide_num = int(os.path.basename(slide_filepath).split('.svs')[0])  ## ap
    ## slide_filepath = get_training_slide_path(slide_num)  ## ap
...
def slide_info(datapath, display_all_properties=False):  ## ap
  """
